...
- Dotfiles Ant default excludes would drop from the jar are stored under a neutral name in templates and renamed back on render — `.gitignore` → `gitignore.txt`, `.gitattributes` → `gitattributes.txt`. The table is `templateRenamedFiles` in `build.gradle.kts` (forward) and `RESTORED_FILE_NAMES` in `TemplateRenderer.kt` (reverse); like the substitution tables, the two must stay in sync. The `gradlew` executable bit is restored on render (jars can't carry POSIX modes). `generateTemplates` fails fast if any template path would be eaten by Ant default excludes.
